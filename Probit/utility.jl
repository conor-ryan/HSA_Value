...
function numDeriv!(grad::Vector{Float64},func::Function,x::Vector{Float64};ϵ=1e-6)
    val = func(x)
    ...
    for i in 1:length(x)
        ...
        if (grad_l>0) & (grad_l>grad_r)
            grad[i] = -grad_l
        end
        if val_y>val
            println("Higher LL at index $i, positive deviation: $val_y")
        end
        if val_z>val
            println("Higher LL at index $i, negative deviation: $val_z")
        end
        println("Index $i, right grad: $grad_r, left grad: $grad_l")
    end
end
